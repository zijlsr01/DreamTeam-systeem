...
get_header(); 
		...
		//aanhakers toevoegen
			...
			if($_POST[addMid]){
				...
				$isAlAanhaker = $wpdb->get_row( "SELECT * FROM Aanhakers WHERE Persbericht LIKE '%".$actID."%' " );
				if(empty($isAlAanhaker)){
				$wpdb->insert( Aanhakers, array( 'Social' => $social, 'Radiointerview' => $radio, 'Persbericht' => $actID, 'Special' => $special, 'Campagne' => $campagne, 'Event' => $event ) );
				$al = "nee";
				}else{
					$wpdb->update( Aanhakers, array( 'Social' => $social, 'Radiointerview' => $radio, 'Persbericht' => $actID, 'Special' => $special, 'Campagne' => $campagne, 'Event' => $event  ),array( 'ID' => $isAlAanhaker->ID) );
				$al = 'ja';
				}
	...
	<div id="contentLinks">
		<div style="height: 5px; width: 590px; background: #f58d4d; float: left;">&nbsp;</div>
		...
		<div class="contentHeaderIcon"><img src="<?php bloginfo('template_url'); ?>/images/iconPersbericht.png" width="30"></div>
			...
			<div class="edit"><?php if(!isset($_GET['edit'])){ ?><a href="/drpr/detailpagina-persbericht/?id=<?php echo $actDetails->ID; ?>&edit=1" ><img src="<?php bloginfo('template_url'); ?>/images/edit.png" alt="" title="Bewerk de gegevens" /></a><?php } ?><a href="/drpr/contentkalender?delete4=yes&id=<?php echo $actDetails->ID; ?>" ><img src="<?php bloginfo('template_url'); ?>/images/iconDelete.png" alt="" title="Verwijderen" onclick="return confirm('Weet je zeker dat je dit persbericht wilt verwijderen?')"/></a></div>
		</div>
		...
		?>
			...
			?>
				...
				<form action="https://www.expect-webmedia.nl/drpr/detailpagina-persbericht/?id=<?php echo $_GET['id'];?>&show=1 " method="post">
				Titel *<br />
	...
	<?php if($actDetails->type == 'Campagne'){ ?>
		...
		<?php 
				global $wpdb;
				$vandaag = date('Ymd');
				$activiteiten = $wpdb->get_results( "SELECT * FROM Aanhakers WHERE Persbericht LIKE '%".$actID."%'" );
				?>
		...
		<div class="contentContentdr">
			...
			?>
					<a href="/drpr/detailpagina-persbericht?id=<?php echo $actID; ?>&add=1&chanel=<?php echo $kanaal; ?>#<?php echo $kanaalDetails->Kanaal; ?>"><img src="<?php bloginfo('template_url'); ?>/images/buttonToevoegenAanhaker.png" border="0"></a>
					<?php } ?>
					<?php if(!empty($_GET['add']) && $_GET['chanel'] == $kanaal){ ?>
					<div style="background: #f37021; padding: 10px; border-radius: 10px;" class="addMid">
					<img src="<?php bloginfo('template_url'); ?>/images/buttonToevoegenAanhaker.png" border="0">
						<form action="https://www.expect-webmedia.nl/drpr/detailpagina-persbericht/?id=<?php echo $_GET['id'];?>&show=2 " method="post">
						
						<!-- Speciale dagen -->
						<b>Speciale dag(en)</b><br />
						<table cellpadding="0" cellspacing="0" border="0">
						<?php 
						$uitsluitenSpecial= $wpdb->get_row("SELECT * FROM Aanhakers WHERE Persbericht LIKE '%".$actID."%'");
						...
						<table cellpadding="0" cellspacing="0" border="0">
						<?php 
						$uitsluitenRadio = $wpdb->get_row("SELECT * FROM Aanhakers WHERE Persbericht LIKE '%".$actID."%'");
						...
						<table cellpadding="0" cellspacing="0" border="0">
						<?php 
						$uitsluitenSocial = $wpdb->get_row("SELECT * FROM Aanhakers WHERE Persbericht LIKE '%".$actID."%'");
						...
						<table cellpadding="0" cellspacing="0" border="0">
						<?php 
						$uitsluitenCampagne = $wpdb->get_row("SELECT * FROM Aanhakers WHERE Persbericht LIKE '%".$actID."%'");
						...
						<!-- /campagnes -->
							...
							<br /><br />
						<a href="/drpr/detailpagina-persbericht/?id=<?php echo $actDetails->ID; ?> ">Annuleren</a> <input type="submit" id="submit2" name="addMid" value="Verzend!" style="cursor: pointer;">
						</form>
					</div>
					<?php } ?>
			<table border="0" cellpadding="5" cellspacing="3">
			<?php
				...
				$aanhaker = $wpdb->get_row("SELECT * FROM Aanhakers WHERE FIND_IN_SET('$actID', Persbericht) > 0;");
				if(empty($aanhaker)){
					...
					<?php
						...
						<tr bgcolor="#8b8b8b" style="color: #ffffff;">
						<td valign="top">
						&nbsp;<a style="color: #ffffff;" href="https://www.expect-webmedia.nl/drpr/detailpagina-radiointerview/?id=<?php echo $radioData->ID; ?>"><?php echo $radioData->Titel." (".get_mooiedatum($radioData->Tijd_van).")";  ?></a>
						</td>
						</tr>
						</table>
					<?php } 
