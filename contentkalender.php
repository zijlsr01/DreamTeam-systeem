...
get_header();
				...
				if(!empty($_GET['delete6'])){
					//$actDetails = $wpdb->get_row("SELECT * FROM Event WHERE ID = ".$_GET['id']."" );
					$wpdb->query(   "  DELETE FROM Event WHERE ID = ".$_GET['id']." "  );
				?>
			
				<div id="s6" style="width: 1150px;  background-color: #007942; float: left; margin-bottom: 15px;padding: 20px; color: #ffffff;">
					Intern evenement is met succes verwijderd!
				</div>
				<?php }
				...
				<div id="addMenux">
					...
					<div id="cam"><a href="/drpr/content-toevoegen/">Campagne</a></div>
					<div id="soc"><a href="/drpr/social-media-toevoegen/">Social Media</a></div>
					<div id="per"><a href="/drpr/persbericht-toevoegen/">Persbericht</a></div>
					<div id="rad"><a href="/drpr/radio-interview-toevoegen/">Radio Interview</a></div>
					<div id="spec"><a href="/drpr/speciale-dagen-toevoegen/">Speciale dag(en)</a></div>
					<?php } ?>
					<div id="event"><a href="/drpr/contentkalender/event-toevoegen/">Intern evenement</a></div>
				</div>
				<table cellspacing="0" cellpadding="10" border="0" width="100%">
					<tr>
						<td width="33%"><a href="/drpr/contentkalender/?month=<?php echo $vorigeMaand; ?>&jaar=<?php echo $vorigeJaar; ?>" style="color: #393939;">&laquo; <?php echo $vorigeMooieMaand." ".$vorigeJaar; ?></a></td>
						<td width="33%" style="text-align: center;"><span class="month"><?php echo $huidigeMaand." ".$jaar; ?></span></td>
						<td width="33%" valign="right" style="text-align: right;"><a href="/drpr/contentkalender/?month=<?php echo $volgendeMaand; ?>&jaar=<?php echo $volgendeJaar; ?>" style="color: #393939;"><?php echo $volgendeMooieMaand." ".$volgendeJaar; ?> &raquo;</a></td>
					</tr>
				</table>
						...
						?><td height="10" valign="bottom"><?php
							...
							foreach($specials as $special){
								if($special->Tijd_van > $specialData || $special->Tijd_tot < $specialData){  }else{ ?>
									<div onclick="location.href='/drpr/contentdetail-speciale-dagen/?id=<?php echo $special->ID; ?>'" style="width: 100%; height: 10px; background-color: <?php echo $special->kleur; ?>; border-bottom: 5px solid #fff; cursor: pointer;" title="<?php echo $special->Titel; ?>"></div>
								<?php }
				...
				foreach(range(1, $d) as $data){
						...
						?><td height="30" valign="bottom" style="border-right: 2px solid #fff;"><?php
							$specials = $wpdb->get_results( "SELECT * FROM Event WHERE (`Tijd_van` BETWEEN '".$specialBegin."' AND '".$specialEind."') AND (`Tijd_tot` BETWEEN '".$specialBegin."' AND '".$specialEind."')" );
							$specialData = $jaar.$maand.$dag;
							foreach($specials as $special){
								if($special->Tijd_van > $specialData || $special->Tijd_tot < $specialData){  }else{ ?>
									<div  onclick="location.href='/drpr/contentkalender/detail-intern-evenement/?id=<?php echo $special->ID; ?>'" style="width: 100%; height: 30px; border-right: 1px solid #fff; background-color: #56e500; border-bottom: 2px solid #fff; cursor: pointer;" title="Intern event: <?php echo $special->Titel; ?>" class="ev"></div>
								<?php }
				...
				?> <tr> <?php 
							foreach( $datas as $data){
									...
									?><td <?php echo $style; ?> title="<?php echo $campagne->type.": ".$campagne->Titel; ?>" class="campagne" ><?php if( $camStart == $currentDate){ ?><div class="overLay"  <?php if($currentLevel != 0){ ?>onclick="location.href='/drpr/contentdetail/?id=<?php echo $campagne->ID; ?>'"<?php } ?>><?php echo getIcon($campagne->type);?><?php if($som > $currentDate){ ?>&nbsp; <div class="overLayText"><?php echo $campagne->type.": ".$campagne->Titel; } ?></div></div><?php } ?><?php if( $camStart < $beginMaand && $currentDay == 01){ ?><div class="overLay" onclick="location.href='/drpr/contentdetail/?id=<?php echo $campagne->ID; ?>'"><?php echo getIcon($campagne->type);?>&nbsp; <?php if($camEind >= $korteCam){ ?><div class="overLayText"><?php echo $campagne->type.": ".$campagne->Titel; ?></div><?php } ?></div><?php } ?></td><?
									$style ="";
								}else{
				...
				foreach(range(1, $d) as $data){
						...
						?><td height="30" valign="bottom" style="border-right: 2px solid #fff;"><?php
							...
							foreach($specials as $special){
								if($special->Tijd_van > $specialData || $special->Tijd_tot < $specialData){  }else{ ?>
									<div  onclick="location.href='/drpr/contentdetail-social-media-post/?id=<?php echo $special->ID; ?>'" style="width: 100%; height: 30px; border-right: 1px solid #fff; background-color: #5ea9dd; border-bottom: 2px solid #fff; cursor: pointer;" title="<?php echo getKanaalName($special->KanaalID); ?>: <?php echo $special->Titel; ?>" class="social"></div>
								<?php }
				...
				foreach(range(1, $d) as $data){
						...
						?><td height="30" valign="bottom" style="border-right: 2px solid #fff;"><?php
							...
							foreach($specials as $special){
								if($special->Tijd_van > $specialData || $special->Tijd_tot < $specialData){  }else{ ?>
									<div  <?php if($currentLevel != 0){ ?>onclick="location.href='/drpr/detailpagina-persbericht/?id=<?php echo $special->ID; ?>'"<?php } ?> style="width: 100%; height: 30px; border-right: 1px solid #fff; background-color: #f58d4d; border-bottom: 2px solid #fff; cursor: pointer;" title="Persbericht: <?php echo $special->Titel; ?>" class="pers"></div>
								<?php }
				...
				foreach(range(1, $d) as $data){
						...
						?><td height="30" valign="bottom" style="border-right: 2px solid #fff;"><?php
							...
							foreach($specials as $special){
								if($special->Tijd_van > $specialData || $special->Tijd_tot < $specialData){  }else{ ?>
									<div  <?php if($currentLevel != 0){ ?>onclick="location.href='/drpr/detailpagina-radiointerview/?id=<?php echo $special->ID; ?>'" <?php } ?> style="width: 100%; height: 30px; border-right: 1px solid #fff; background-color: #00ab4e; border-bottom: 2px solid #fff; cursor: pointer;" title="Persbericht: <?php echo $special->Titel; ?>" class="radio"></div>
								<?php }
